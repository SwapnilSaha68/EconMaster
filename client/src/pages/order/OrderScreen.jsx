...
const OrderScreen = () => {
  ...
  const wishlistNotification = useSelector((state) => state.wishlistNotification);
  ...
  useEffect(() => {
    ...
    if (wishlistNotification) {
      setAdminNotification(wishlistNotification);
    }
  }, [
    dispatch,
    order,
  ...
  return (
    <Layout>
      ...
      <div className="py-10 px-5">
        {/* Display admin notification for wishlist */}
        {adminNotification && (
          <Alert variant="info">{adminNotification}</Alert>
        )}
              ...
              <div className="space-y-2">
                <div className="flex justify-between item-center">
                  ...
                  <span className="text-lime-700 font-bold">
                    BDT {addDecimals(order.itemsPrice)}
                  </span>
                </div>
                <div className="flex justify-between item-center">
                  <span className="text-gray-500">Shipping</span>
                  <span className="text-lime-700 font-bold">
                    BDT {addDecimals(order.shippingPrice)}
                  </span>
                </div>
                <div className="flex justify-between item-center">
                  <span className="text-gray-500">Tax</span>
                  <span className="text-lime-700 font-bold">
                    BDT {addDecimals(order.taxPrice)}
                  </span>
                </div>
                <div className="flex justify-between item-center">
                  <span className="text-gray-500">Total</span>
                  <span className="text-lime-700 font-bold">
                    BDT {addDecimals(order.totalPrice)}
                  </span>
                </div>
              </div>
