...
const PlaceOrderScreen = () => {
  ...
  // Local state to manage order placement
  ...
  }, [success, navigate, paymentMethod, order]);
  ...
  const payHandler = () => {
    // Implement your Bkash payment logic here
    // For example, redirect to Bkash payment gateway
    // You might need to create a payment intent on the server and get a payment URL
    navigate(`/payment/bkash?orderId=${order._id}`);
  };
        ...
        <div className="w-full flex flex-col lg:flex-row lg:justify-start lg:space-x-5 mt-5 lg:mt-10">
          ...
          <div className="w-full flex flex-col font-medium mt-8 lg:mt-0 lg:w-80">
            <div className="space-y-2">
              <div className="flex justify-between item-center">
                ...
                <span className="text-lime-700 font-bold">
                  BDT {addDecimals(itemsPrice)}
                </span>
              </div>
              <div className="flex justify-between item-center">
                <span className="text-gray-500">Shipping</span>
                <span className="text-lime-700 font-bold">
                  BDT {addDecimals(shippingPrice)}
                </span>
              </div>
              <div className="flex justify-between item-center">
                <span className="text-gray-500">Tax</span>
                <span className="text-lime-700 font-bold">
                  BDT {addDecimals(taxPrice)}
                </span>
              </div>
              <div className="flex justify-between item-center">
                <span className="text-gray-500">Total</span>
                <span className="text-lime-700 font-bold">
                  BDT {addDecimals(totalPrice)}
                </span>
              </div>
            </div>
            ...
            {orderPlaced && paymentMethod === "Bkash" && (
              <button
                className="w-full text-white bg-green-600 px-5 py-3 mt-4"
                onClick={payHandler}
              >
                Pay with Bkash
              </button>
